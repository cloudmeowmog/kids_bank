...
with tab1:
    ...
    with col1:
        # 當切換帳戶時，重置刪除狀態
        target = st.selectbox("檢視帳戶", st.session_state['children'], on_change=lambda: st.session_state.update({'confirm_delete': False, 'delete_idx': None}))
    ...
    if user_data['history']:
        ...
        if selected_record_str:
            ...
            # 將刪除按鈕移出 form 外，避免送出表單的行為衝突
            st.write("**或刪除此筆紀錄：**")
            col_del1, col_del2 = st.columns([1, 4])
            
            with col_del1:
                 # 第一階段：點擊刪除
                 if st.button("🗑️ 刪除此紀錄", key="btn_del_init"):
                     st.session_state['confirm_delete'] = True
                     st.session_state['delete_idx'] = idx
            
            with col_del2:
                # 第二階段：確認刪除
                if st.session_state.get('confirm_delete') and st.session_state.get('delete_idx') == idx:
                    st.warning("⚠️ 確定要刪除嗎？刪除後無法復原！")
                    if st.button("🚨 確認永久刪除", key="btn_del_confirm", type="primary"):
                        # 執行刪除
                    ...
                    if st.button("❌ 取消刪除", key="btn_del_cancel"):
                         st.session_state['confirm_delete'] = False
                         st.session_state['delete_idx'] = None
                         st.rerun()
